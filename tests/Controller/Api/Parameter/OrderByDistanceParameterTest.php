...
class OrderByDistanceParameterTest extends AbstractApiControllerTest
{
     ...
     */
    public function testResultListOrderByAscending(string $fqcn): void
    {
        $client = static::createClient();

        $hamburg = Coords::hamburg();

        $uri = sprintf('%s?centerLatitude=%f&centerLongitude=%f&distanceOrderDirection=ASC', $this->getApiEndpointForFqcn($fqcn), $hamburg->getLatitude(), $hamburg->getLongitude());
        ...
        foreach ($resultList as $result) {
            $distance = DistanceCalculator::calculateDistance($hamburg, $result->toCoord());
     ...
     * @dataProvider apiClassProvider
     */
    public function testResultListOrderByDescending(string $fqcn): void
    {
        $client = static::createClient();

        $hamburg = Coords::hamburg();

        $uri = sprintf('%s?centerLatitude=%f&centerLongitude=%f&distanceOrderDirection=DESC', $this->getApiEndpointForFqcn($fqcn), $hamburg->getLatitude(), $hamburg->getLongitude());
