...
class RideEstimateRepository extends EntityRepository
{
    public function findForTimelineRideParticipantsEstimateCollector(\DateTime $startDateTime = null, \DateTime $endDateTime = null, $limit = null)
    {
        ...
        if ($startDateTime) {
            $builder->andWhere($builder->expr()->gte('estimate.dateTime', '\'' . $startDateTime->format('Y-m-d H:i:s') . '\''));
        }

        if ($endDateTime) {
            $builder->andWhere($builder->expr()->lte('estimate.dateTime', '\'' . $endDateTime->format('Y-m-d H:i:s') . '\''));
        }
        ...
        $builder->addOrderBy('estimate.dateTime', 'DESC');
