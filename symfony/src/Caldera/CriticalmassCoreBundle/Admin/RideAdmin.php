...
class RideAdmin extends Admin
{
    ...
    {
        $formMapper
            ->with('Stadt', array('class' => 'col-md-6', 'description' => 'Wähle hier die Stadt, in der diese Tour stattfindet.'))
                ->add('city', null, array('label' => 'Stadt', 'required' => true))
            ->end()
            ->with('Zusatzinformationen', array('class' => 'col-md-6', 'description' => 'Optionale Zusatzangaben zur Tour. Werden diese Felder nicht ausgefüllt, wird als Titel „Critical Mass“ verwendet, eine Beschreibung wird nicht angezeigt.'))
                ->add('title', 'text', array('label' => 'Titel', 'required' => false))
                ->add('description', 'textarea', array('label' => 'Beschreibung', 'required' => false))
            ->end()
            ->with('Datum und Uhrzeit', array('class' => 'col-md-6', 'description' => 'Lege hier das Datum und die Uhrzeit der Tour fest.'))
            ...
            ->end()
            ->with('Sichtbarkeit', array('class' => 'col-md-6', 'description' => 'Da die meisten Touren automatisch generiert werden, legt dieser Sichtbarkeitsbereich fest, in welchem Zeitraum sie auf der Karte angezeigt werden. Normalerweise sollte eine Tour noch eine Woche nach ihrem Termin auf der Karte verbleiben, anschließend wird die jeweils nächste Tour angezeigt.'))
                ->add('visibleSince', 'datetime', array('label' => 'Sichtbar ab'))
                ->add('visibleUntil', 'datetime', array('label' => 'Sichtbar bis'))

            ->end()
            ->with('Treffpunkt', array('class' => 'col-md-6', 'description' => 'Der Treffpunkt wird mit der Angabe des Längen- und Breitengrades festgestellt. Nutze <a href="http://itouchmap.com/latlong.html">iTouchMap</a>, um diese Werte zu ermitteln.'))
            ...
            ->end()
            ->with('Social Media', array('class' => 'col-md-6', 'description' => 'Verlinke hier auf soziale Netzwerke, auf denen sich die Teilnehmer über die Tour informieren können.'))
            ...
            ->end()
            ->with('Statistik', array('class' => 'col-md-6', 'description' => 'In einer späteren Version von criticalmass.in werden die Teilnehmer Schätzungen über die Touren abgeben können, deren Mittelwerte für statistische Werte einer Tour benutzt werden. Mit den folgenden Werten lassen sich die Schätzungen der Benutzer überschreiben. Bedenke, dass in den folgenden Eingabefeldern keine Angaben der Einheiten zulässig sind.'))
            ...
            ->end()
            ->with('Teaserbild', array('class' => 'col-md-6'))
